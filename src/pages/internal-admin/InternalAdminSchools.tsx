...
import { internalAdminApi, School, SchoolSubscription } from '@/services/internalAdminApi';
import { Search, CreditCard, Eye } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';

const InternalAdminSchools = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["internal-admin", "schools", page, search],
    queryFn: () => internalAdminApi.getSchools(page, 50, search),
  });

  const { data: subscriptionData } = useQuery({
    queryKey: ["internal-admin", "school-subscription", selectedSchool?.id],
    queryFn: () => selectedSchool ? internalAdminApi.getSchoolSubscription(selectedSchool.id) : null,
    enabled: !!selectedSchool,
  });

  const toggleMutation = useMutation({
    mutationFn: ({ schoolId, isActive }: { schoolId: string; isActive: boolean }) =>
      internalAdminApi.toggleSchoolStatus(schoolId, isActive),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["internal-admin", "schools"] });
      toast({
        title: "Success",
...
export default InternalAdminSchools;
