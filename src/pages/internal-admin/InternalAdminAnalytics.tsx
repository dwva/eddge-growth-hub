...
import { internalAdminApi, UsageAnalytics } from '@/services/internalAdminApi';
import { TrendingUp, Zap, Brain, RefreshCw, AlertCircle } from 'lucide-react';
import { format, subDays } from 'date-fns';

const InternalAdminAnalytics = () => {
  const [startDate, setStartDate] = useState(format(subDays(new Date(), 30), 'yyyy-MM-dd'));
  const [endDate, setEndDate] = useState(format(new Date(), 'yyyy-MM-dd'));

  const { data, isLoading, error, refetch, isRefetching } = useQuery<UsageAnalytics>({
    queryKey: ["internal-admin", "analytics", startDate, endDate],
    queryFn: () => internalAdminApi.getUsageAnalytics(startDate, endDate),
  });
...
export default InternalAdminAnalytics;
