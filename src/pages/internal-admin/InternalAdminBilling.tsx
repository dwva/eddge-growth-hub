...
import { internalAdminApi, BillingPlan, BillingRenewal, Invoice } from '@/services/internalAdminApi';
import { CreditCard, Calendar, CheckCircle2 } from 'lucide-react';

const InternalAdminBilling = () => {
  const [activeTab, setActiveTab] = useState('plans');
  const [invoicePage, setInvoicePage] = useState(1);

  const { data: plansData, isLoading: plansLoading } = useQuery<BillingPlan[]>({
    queryKey: ["internal-admin", "billing-plans"],
    queryFn: () => internalAdminApi.getBillingPlans(),
  });

  const { data: renewalsData, isLoading: renewalsLoading } = useQuery<BillingRenewal[]>({
    queryKey: ["internal-admin", "billing-renewals"],
    queryFn: () => internalAdminApi.getBillingRenewals(30),
  });

  const { data: invoicesData, isLoading: invoicesLoading } = useQuery({
    queryKey: ["internal-admin", "billing-invoices", invoicePage],
    queryFn: () => internalAdminApi.getBillingInvoices(invoicePage, 50),
  });
...
export default InternalAdminBilling;
